...
### (2) 다음 작업: 네모 모드에서 유튜브 컨트롤 클릭 — **방법 A 승인됨, 미구현**
- 문제: 네모 모드는 페이지 클릭을 전부 가로채 **재생바 seek·버튼 클릭이 유튜브에 안 닿음**.
- **승인된 방향(A)**: "단순 클릭은 유튜브로 통과 / 드래그만 캡처". 구현 시 — mousedown에서 preventDefault·stopImmediatePropagation을 *즉시* 하지 말고, mousemove에서 이동 임계값(예: 4~8px) 넘어 **드래그 확정 시에만** 억제 시작. 단순 클릭(이동 없음)은 그대로 통과시켜 유튜브가 seek/버튼/재생 처리. `swallowPointer`(pointerdown/up 캡처 차단)도 *드래그 중에만* 작동하게 완화. 드래그 끝엔 기존 `suppressClick`으로 트레일링 click 무효화 유지.
- ⚠ **리스크**: 예전에 고친 "정지 영상 드래그하면 재생됨" 버그가 재발할 수 있음(유튜브가 누름(press)에 반응하는지 클릭에 반응하는지에 좌우 — 실측 필요). 적용 후 **반드시 실유튜브에서 "정지영상 드래그 시 재생 안 됨" 재확인.** 재발하면 드래그 중 press까지 차단하도록 보강하거나 방법 B(유튜브 컨트롤 셀렉터만 통과)/C(현행)로 후퇴.
- 방법 A를 넣으면 (1)의 "클릭→재생토글" 자체 처리는 유튜브 네이티브 클릭이 대신하므로 **제거/단순화** 가능.
